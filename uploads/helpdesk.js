...
async function loadTickets() {
  // Tentar Supabase via Flask primeiro
  try {
    ...
    if (r.ok) {
      ...
      if (data && data.length > 0) {
        tickets = data.map(t => ({
          ...t,
          historico: typeof t.historico === 'string' ? JSON.parse(t.historico || '[]') : (t.historico || []),
          tags: t.tags || [],
          criado: t.criado_em || t.criado,
          atualizado: t.atualizado_em || t.atualizado,
        }));
...
function getDemoTickets() {
  return [
    { id:101, protocolo:"#101", titulo:"Nova Admissão | Santa Catarina | 22/06/2026",
      ...
      tipo_equip:"desktop", patrimonio:"", tags:["admissão","novo colaborador"],
    ...
    { id:102, protocolo:"#102", titulo:"Bloqueio de Acessos - Realty",
      ...
      tipo_equip:"desktop", patrimonio:"", tags:["bloqueio","desligamento"],
      ...
      historico:[
        ...
        {autor:"Técnico 1",tipo:"resposta",texto:"Iniciando processo de bloqueio.",data:new Date(Date.now()-3600000).toISOString()}
      ]
    },
  ];
}
  ...
  loadTickets().then(() => {
    ...
    document.getElementById("pageContent").innerHTML = `
      <div class="stats-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:18px">
        <div class="stat-card blue"><div class="stat-accent"></div><div class="stat-icon-wrap"><i class="fa-solid fa-ticket"></i></div><div class="stat-label">Total</div><div class="stat-num">${tickets.length}</div></div>
        <div class="stat-card amber"><div class="stat-accent"></div><div class="stat-icon-wrap"><i class="fa-solid fa-clock"></i></div><div class="stat-label">Abertos</div><div class="stat-num">${statCounts("aberto")}</div></div>
        <div class="stat-card purple"><div class="stat-accent"></div><div class="stat-icon-wrap"><i class="fa-solid fa-spinner"></i></div><div class="stat-label">Em andamento</div><div class="stat-num">${statCounts("andamento")}</div></div>
        <div class="stat-card green"><div class="stat-accent"></div><div class="stat-icon-wrap"><i class="fa-solid fa-circle-check"></i></div><div class="stat-label">Resolvidos</div><div class="stat-num">${statCounts("resolvido")}</div></div>
      </div>
      ...
      <div class="table-wrap">
        <div class="ticket-list-header">
          <div style="min-width:70px">Nº</div><div style="width:32px"></div>
          <div style="flex:1">Assunto</div><div style="min-width:100px">Categoria</div>
          <div style="min-width:80px">Agente</div><div style="min-width:90px">Prioridade</div>
          <div style="min-width:90px">Status</div><div style="min-width:80px">Criado</div>
        </div>
...
function renderTicketList() {
  ...
  const list = tickets.filter(t =>
    (sf==="all"||t.status===sf) && (pf==="all"||t.prioridade===pf) && (cf==="all"||t.categoria===cf)
  ).sort((a,b)=>new Date(b.criado||b.criado_em)-new Date(a.criado||a.criado_em));
  ...
  body.innerHTML = list.map(t => {
    ...
    return `<div class="ticket-row ${t.prioridade}" onclick="openTicketDetail(${t.id})">
      ...
      <div class="ticket-time">${relativeTime(t.criado||t.criado_em)}</div>
    </div>`;
  }).join("");
}
...
function openTicketDetail(id) {
  ...
  document.getElementById("ticketModalSub").textContent = t.solicitante + " · " + relativeTime(t.criado||t.criado_em);
  const initials = (t.solicitante||"?").split(" ").map(w=>w[0]).slice(0,2).join("").toUpperCase();
  ...
  document.getElementById("ticketModalBody").innerHTML = `
    <div class="ticket-detail-layout">
      <div class="ticket-detail-main">
        <div class="ticket-actions-bar">
          <button class="ticket-action-btn primary" onclick="setReplyTab('responder')"><i class="fa-solid fa-reply"></i> Responder</button>
          <button class="ticket-action-btn" onclick="setReplyTab('anotacao')"><i class="fa-solid fa-note-sticky"></i> Anotação</button>
          <button class="ticket-action-btn" onclick="setReplyTab('encaminhar')"><i class="fa-solid fa-share"></i> Encaminhar</button>
          <button class="ticket-action-btn" onclick="resolveTicket(${t.id})" style="margin-left:auto"><i class="fa-solid fa-circle-check"></i> Fechar</button>
        </div>
        <div class="ticket-email-header">
          ...
          <div class="ticket-email-from" style="margin-top:10px">
            ...
            <div>
              ...
              <div class="ticket-email-meta">relatado · ${new Date(t.criado||t.criado_em||Date.now()).toLocaleString("pt-BR")}</div>
        ...
        <div class="timeline" style="margin-top:18px">
          ${hist.map(h=>`<div class="timeline-item">
            <div><div class="timeline-dot ${h.tipo==='sistema'?'system':''}"></div></div>
        ...
        <div class="reply-box">
          ...
          <div class="reply-footer">
            <span style="font-size:12px;color:var(--text-3)"><i class="fa-solid fa-lock"></i> Para ${t.email||t.solicitante}</span>
            <button class="btn-primary btn-sm" onclick="sendReply(${t.id})"><i class="fa-solid fa-paper-plane"></i> Enviar</button>
          </div>
        </div>
      </div>
      <div class="ticket-detail-sidebar">
        ...
        <div class="ticket-sidebar-section">
          ...
          <div class="ticket-prop"><label>Tipo</label>
          ...
          <div class="ticket-prop"><label>Status</label>
            <select onchange="updateTicketProp(${t.id},'status',this.value)">
              ${["aberto","andamento","resolvido","fechado"].map(s=>`<option value="${s}" ${t.status===s?"selected":""}>${{aberto:"Aberto",andamento:"Em andamento",resolvido:"Resolvido",fechado:"Fechado"}[s]}</option>`).join("")}
            </select>
          </div>
          <div class="ticket-prop"><label>Prioridade</label>
            <select onchange="updateTicketProp(${t.id},'prioridade',this.value)">
              ${["baixa","media","alta","critica"].map(p=>`<option value="${p}" ${t.prioridade===p?"selected":""}>${{baixa:"Baixa",media:"Média",alta:"Alta",critica:"Crítica"}[p]}</option>`).join("")}
            </select>
          </div>
          <div class="ticket-prop"><label>Grupo</label>
            <select onchange="updateTicketProp(${t.id},'grupo',this.value)">
              <option value="">Selecione...</option>
              ${cfg.grupos.map(g=>`<option value="${g}" ${t.grupo===g?"selected":""}>${g}</option>`).join("")}
            </select>
          </div>
          <div class="ticket-prop"><label>Agente</label>
          ...
          ${t.patrimonio?`<div class="ticket-prop"><label>Patrimônio</label>
        ...
        <div class="ticket-sidebar-section">
          ...
          ${tickets.filter(x=>x.id!==t.id&&x.email===t.email).slice(0,3).map(x=>`
            <div onclick="openTicketDetail(${x.id})" style="cursor:pointer;padding:8px;border-radius:var(--radius);margin-bottom:6px;background:var(--surface-2);border:1px solid var(--border)">
              ...
              <div style="font-size:11px;color:var(--text-3)">${x.protocolo} · ${relativeTime(x.criado||x.criado_em)}</div>
    ...
    <div style="display:flex;justify-content:space-between;margin-top:16px;padding-top:14px;border-top:1px solid var(--border)">
      <button class="btn-danger btn-sm" onclick="deleteTicketConfirm(${t.id})"><i class="fa-solid fa-trash"></i> Excluir</button>
...
function setReplyTab(tab) {
  ...
  const ph = {responder:"Digite sua resposta...",anotacao:"Anotação interna (visível apenas para a equipe)...",encaminhar:"E-mail de destino e mensagem..."};
  if (ta) ta.placeholder = ph[tab]||"";
}
...
async function sendReply(id) {
  ...
  const entry = { autor:currentUser?.nome||currentUser?.username||"Técnico", tipo:"resposta", texto:text, data:new Date().toISOString() };
  if (!t.historico) t.historico = [];
  t.historico.push(entry);
  t.atualizado = t.atualizado_em = new Date().toISOString();

  // Salvar nos dois lugares
  try {
    await fetch(`${API_BASE}/api/tickets/${id}`, {
      method:"PUT", headers:{"Content-Type":"application/json"},
      body:JSON.stringify({historico_add:entry})
    });
  } catch {}
  saveTickets();
  openTicketDetail(id);
...
async function updateTicketProp(id, prop, value) {
  const t = tickets.find(x=>x.id===id);
  if (!t) return;
  const old = t[prop];
  t[prop] = value;
  t.atualizado = t.atualizado_em = new Date().toISOString();
  const entry = {autor:"Sistema",tipo:"sistema",texto:`${prop} alterado de "${old}" para "${value}".`,data:new Date().toISOString()};
  if (!t.historico) t.historico=[];
  t.historico.push(entry);
  ...
  try {
    await fetch(`${API_BASE}/api/tickets/${id}`, {
      method:"PUT", headers:{"Content-Type":"application/json"},
      body:JSON.stringify({[prop]:value, historico_add:entry})
    });
  } catch {}
  saveTickets();
...
async function deleteTicketConfirm(id) {
  ...
  try { await fetch(`${API_BASE}/api/tickets/${id}`,{method:"DELETE"}); } catch {}
  ...
  document.getElementById("pageContent").innerHTML = `
    <div style="max-width:700px">
      <div class="card" style="padding:28px">
        ...
        <div class="form-grid">
          <div class="form-field span2"><label>Título / Assunto *</label><input type="text" id="nt-titulo" autocomplete="off" placeholder="Ex: Nova Admissão | Livus SC | 22/06/2026"></div>
          <div class="form-field"><label>Nome do solicitante *</label><input type="text" id="nt-solicitante" autocomplete="off" placeholder="Nome completo"></div>
          <div class="form-field"><label>E-mail *</label><input type="email" id="nt-email" autocomplete="off" placeholder="email@empresa.com"></div>
          <div class="form-field"><label>Setor / Empresa</label><input type="text" id="nt-setor" autocomplete="off" placeholder="RH, Financeiro..."></div>
          <div class="form-field"><label>Categoria / Tipo</label>
            <select id="nt-categoria"><option value="">Selecione...</option>${cfg.categorias.map(c=>`<option value="${c}">${c}</option>`).join("")}</select>
          </div>
          <div class="form-field"><label>Prioridade</label>
            <select id="nt-prioridade"><option value="baixa">Baixa</option><option value="media" selected>Média</option><option value="alta">Alta</option><option value="critica">Crítica</option></select>
          </div>
          <div class="form-field"><label>Grupo</label>
            <select id="nt-grupo"><option value="">Selecione...</option>${cfg.grupos.map(g=>`<option value="${g}">${g}</option>`).join("")}</select>
          </div>
          <div class="form-field"><label>Agente Responsável</label>
            <select id="nt-agente"><option value="">Não atribuído</option>${cfg.agentes.map(a=>`<option value="${a}">${a}</option>`).join("")}</select>
          </div>
          <div class="form-field"><label>Tipo de Equipamento</label>
            <select id="nt-tipo" onchange="toggleNtPat()">${cfg.tipos_equipamento.map(t=>`<option value="${t}">${t}</option>`).join("")}</select>
          </div>
          <div class="form-field" id="nt-pat-wrap" style="display:none">
            <label><i class="fa-solid fa-tag" style="color:var(--purple);margin-right:5px"></i> Nº Patrimônio</label>
            <input type="text" id="nt-patrimonio" autocomplete="off" placeholder="PAT-000001" style="font-family:monospace;font-weight:600">
          </div>
          <div class="form-field span2"><label>Descrição *</label><textarea id="nt-descricao" rows="6" placeholder="Descreva o chamado com detalhes..."></textarea></div>
        </div>
        <div style="display:flex;justify-content:flex-end;gap:10px;margin-top:20px;padding-top:16px;border-top:1px solid var(--border)">
          ...
          <button class="btn-primary" onclick="criarTicket()"><i class="fa-solid fa-paper-plane"></i> Criar Ticket</button>
        </div>
      </div>
    </div>`;
}
...
async function criarTicket() {
  ...
  const ticketData = {
    titulo, descricao, solicitante, email,
    ...
    criado:ts, criado_em:ts, atualizado:ts, atualizado_em:ts,
  };

  let novoTicket = null;

  // 1. Salvar via Flask → Supabase
  try {
    const r = await fetch(`${API_BASE}/api/tickets`, {
    ...
    if (r.ok) {
      ...
      novoTicket = { ...ticketData, id:d.id, protocolo:d.protocolo||`#${d.id}` };
      toast(`Ticket ${novoTicket.protocolo} criado!`,"success");
    }
  } catch {}
  ...
  if (!novoTicket) {
    ...
    novoTicket = { ...ticketData, id, protocolo:`#${id}` };
    toast(`Ticket #${id} criado localmente.`,"info");
  }

  tickets.unshift(novoTicket);
  nextTicketId = Math.max(nextTicketId, novoTicket.id + 1);
  saveTickets();
  switchView("tickets");
}
